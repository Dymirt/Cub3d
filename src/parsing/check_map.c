	...
	while (map_tab[i] != NULL)
	{
		...
		while (map_tab[i][j])
		{
			...
			if (!(ft_strchr("10NSEW", map_tab[i][j])))
				return (err_msg(data->map_info.path, ERR_INV_LETTER, FAILURE));
			if (ft_strchr("NSEW", map_tab[i][j]) && data->player.dir != '0')
				return (err_msg(data->map_info.path, ERR_NUM_PLAYER, FAILURE));
			if (ft_strchr("NSEW", map_tab[i][j]) && data->player.dir == '0')
...
int	check_map_validity(t_data *data, char **map_tab)
{
	...
	if (check_map_sides(&data->map_info, map_tab) == FAILURE)
		return (err_msg(data->map_info.path, ERR_MAP_NO_WALLS, FAILURE));
	if (data->map_info.height < 3)
		return (err_msg(data->map_info.path, ERR_MAP_TOO_SMALL, FAILURE));
	...
	if (check_map_is_at_the_end(&data->map_info) == FAILURE)
		return (err_msg(data->map_info.path, ERR_MAP_LAST, FAILURE));
	return (SUCCESS);
}
