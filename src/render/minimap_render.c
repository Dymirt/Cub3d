...
static char	*add_minimap_line(t_data *d, t_minimap *m, int y)
{
	...
	if (!line)
		return (NULL);
	x = 0;
	while (x < m->size && x < d->map_info.width)
	{
		if (!is_valid_map_coord(y + m->offset_y, d->map_info.height)
			|| !is_valid_map_coord(x + m->offset_x, d->map_info.width))
			line[x] = '\0';
...
static char	**generate_minimap(t_data *data, t_minimap *minimap)
{
	...
	if (!mmap)
		return (NULL);
	y = 0;
	while (y < minimap->size && y < data->map_info.height)
	{
...
void	render_minimap(t_data *data)
{
	...
	minimap.offset_x = get_mmap_offset(&minimap,
			data->map_info.width, (int)data->player.pos_x);
	minimap.offset_y = get_mmap_offset(&minimap,
			data->map_info.height, (int)data->player.pos_y);
